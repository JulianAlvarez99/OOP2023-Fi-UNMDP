...
bool CFecha::SetYear(unsigned int y)
{
    ...
    try
    {
        if(y > CFecha().GetYear())
        {
            throw ("El a�o es mayor que el actual, intente nuevamente\nAnio: ");
        }
        if(y < 1920)
        {
            throw ("El a�o es muy antiguo, intente nuevamente\nAnio: ");
        }
    }
    catch(const char *codigo)
    {
...
ostream& operator << (ostream& co, CFecha& fecha)
{
    co.fill('0');
    co.width(2);
    co <<right<< fecha.GetDay() << '/';
    co.fill('0');
    co.width(2);
    co <<right<< fecha.GetMonth() << '/'
       <<right<< fecha.GetYear();
...
bool CFecha::operator< (const CFecha fecha) const
{
    ...
    if( this->GetYear() < fecha.GetYear() )
        ...
        cond = false;
    else                                // los anios son iguales
        if( GetMonth() < fecha.GetMonth() )
            cond = true;
        else if( GetMonth() > fecha.GetMonth() )
            cond = false;
        else                                // los anios y los meses son iguales
